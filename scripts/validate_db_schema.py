...
índices e funções esperadas pelas migrations.

Sempre usa REST API (RPC exec_sql_query) para ser compatível com CI/CD.
"""
...
def run_query(client, sql):
    """Executes SQL via the exec_sql_query RPC."""
    try:
        # The RPC returns the result as a list of rows (dicts)
...
def validate_constraints(client):
    print("\n=== CONSTRAINTS ===")
    ok, total = 0, 0
...
def main():
    ...
    results = []
    results.append(validate_tables(client))
    results.append(validate_columns(client))
    results.append(validate_constraints(client))
    results.append(validate_indexes(client))
    results.append(validate_functions(client))
