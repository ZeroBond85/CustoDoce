...
def get_conn():
    if not pwd:
        print("INFO: SUPABASE_DB_PASSWORD not set.")
        print("TIP: Set SUPABASE_DB_PASSWORD in .env or use Supabase SQL Editor.")
        return None
    import psycopg2
    ...
    for host, port, user in [
        ...
        try:
            ...
            continue
    print("WARNING: Cannot connect to Supabase DB directly (port 5432/6543 blocked).")
    print("TIP: Use Supabase SQL Editor for schema inspection, or run from a whitelisted IP.")
    return None
...
# price_history
expected_columns["price_history"] = [
    "id",
    "price_id",
    "ingredient_id",
    "store_id",
...
cur.execute(
    "SELECT proname FROM pg_proc WHERE pronamespace=(SELECT oid FROM pg_namespace WHERE nspname='public') AND proname NOT LIKE 'pgrst%' ORDER BY proname"
)
