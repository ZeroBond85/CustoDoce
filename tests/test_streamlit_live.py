...
def test_streamlit():
    ...
    with sync_playwright() as p:
        ...
        try:
            user_input = page.locator('input[aria-label*="user"], input[aria-label*="usuario"], input[aria-label*="Usuario"], input[aria-label*="User"], input[data-testid="stTextInput"]').first
            if user_input.is_visible(timeout=5000):
                ...
                if pass_input.is_visible(timeout=3000):
                    ...
                    login_btn = page.locator('button:has-text("Entrar"), button:has-text("Login"), button:has-text("Entrar"), button[data-testid="stFormSubmitButton"]').first
        ...
        for i, c in enumerate(clickable):
            try:
                ...
                has_error = "error" in content_text.lower() or "erro" in content_text.lower() or "traceback" in content_text.lower()
