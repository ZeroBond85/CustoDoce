...
class TestTriggerOnConflict:
    ...
    def test_insert_then_update_no_23505(self, real_supabase_client, db_conn):
        ...
        # Insert
        r1 = client.rpc("upsert_price_rpc", {
        ...
        check = client.table("prices").select("raw_price, raw_product").eq(
    ...
    def test_insert_then_update_without_cleanup(self, real_supabase_client, db_conn):
        ...
        check = client.table("prices").select("raw_price").eq(
            "ingredient_id", self.TEST_ING
        ).execute()
...
class TestApproveReviewItem:
    ...
    def test_approve_with_uuid(self, real_supabase_client, db_conn, test_store, test_ingredient):
        ...
        price_check = client.table("prices").select("*").eq(
            "store_id", test_store["id"]
        ).eq("source", "approve_test").execute()
    ...
    def test_approve_duplicate_price_no_23505(self, real_supabase_client, db_conn, test_store, test_ingredient):
        ...
        ).eq("store_id", test_store["id"]).eq("collected_at", today).execute()
        assert len(check.data) == 1, f"Expected 1 price row, got {len(check.data)}"
...
class TestAddAlias:
    ...
    def test_add_alias_to_existing_ingredient(self, real_supabase_client, test_ingredient):
        ...
        test_alias = f"_test_alias_{datetime.now(timezone.utc).strftime('%H%M%S')}"
