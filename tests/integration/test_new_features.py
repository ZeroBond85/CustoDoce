    ...
    def test_no_api_key_returns_none(self):
        ...
        try:
            ...
            result = classifier.classify_sync(
                "Leite Condensado Moça 395g", [{"canonical_name": "Leite Condensado", "aliases": []}]
            )
...
class TestProcessPriceMatchAI:
    ...
    def test_exact_match_auto_approve(self):
        ...
        with patch("services.collector.match_ingredient") as mock_match:
            with patch("services.collector.upsert_price") as mock_upsert:
                mock_match.return_value = (
                    {"canonical_name": "Leite Condensado", "aliases": [], "search_terms": []},
                    95.0,
                    "exato",
                )
    ...
    def test_exact_fuzzy_match_auto_approve(self):
        ...
        with (
            patch("services.collector.match_ingredient") as mock_match,
            patch("services.collector.upsert_price") as mock_upsert,
        ):
    ...
    def test_review_queue_fallback(self):
        ...
        with (
            patch("services.collector.match_ingredient") as mock_match,
            patch("services.collector.rank_ingredients", return_value=[]),
        ):
            with patch("services.collector.insert_review_item") as mock_insert:
