...
- **Nota (numeração)**: a antiga lição **#81** ("Push no Windows suja a working tree (sync_docs auto-fix) e barra o push", 2026-07-17) foi **fundida nesta #80** na compressão p/ 700 linhas (commit `4fc5ca1`). O gap `#81` na numeração é intencional — conteúdo preservado aqui. AGENTS.md regra 15 referencia esta lição.
...
### 97. CI docs-sync quebrava por paridade de ambiente na coleta de testes (2026-08-13)
- **Sintoma/causa**: o job `docs-sync` (ci.yml) instalava só `requirements-dev.lock`, que não tem `pytest-mock`, `pytest-asyncio`, `playwright`, `faker` nem `responses`. `tests/e2e` (o conftest importa playwright) e ~44 testes unit (fixtures `mocker`/`asyncio`) falhavam **na coleta** na CI → contava 1468 testes vs 1597 local. O badge do README (gerado por `sync_docs` a partir de `pytest --collect-only`) divergia → gate `docs-sync` vermelho (`[LIVE] README.md drift`). Era intermitente: um run anterior coletou 1597 (o ambiente da runner variava entre runs).
...
### 113. mypy strict — parity gaps WSL vs Windows: types-PyYAML e transformers stubs (2026-08-26)
Sintoma: mypy --strict no Windows (78 arquivos, 0 erros) mas mypy . no WSL/pre-push falhava com import-untyped para yaml e no-untyped-call para transformers. Causa: types-PyYAML ausente do lock (estava ad-hoc no venv Windows); transformers tem py.typed parcial inconsistente entre versões — per-file --follow-imports=silent mascara o gap. Correção: overrides pyproject.toml — yaml.* em ignore_missing_imports (mesmo padrão streamlit/pandas) e transformers.* com follow_imports=skip (parity-safe, ambas plataformas Any). Regressões descobertas: send_email(to=, body=) errado em scraper_alert.py (TypeError em runtime); cleanup_old_prices retornava 0 sempre (rpc_execute retornava .data direto mas código tratava como result[0].get). Regra: sempre mypy . no WSL antes de push — per-file não valida paridade.
### 114. test_rate_limiter flaky por SQLite compartilhado vazando entre execuções (2026-08-28)
Sintoma: CI unit flake recorrente (run 32659659763 `assert remaining_attempts("persist-test") == 0 == 2`) e falha local intermitente em `-n auto`. Causa raiz: `RateLimiter()` default aponta p/ `data/rate_limiter.db` persistente; o teste registrava `persist-test` e só limpava no FIM — asserção falhou antes → vazou; runs sucessivos dentro do window 60s acumularam (11 timestamps, 3 na janela → remaining=0). Não era race de xdist. Correção (RPR): todos os testes RateLimiter usam `tmp_path`/`tmp_path_factory` (test_dashboard_full x4, test_deploy_check x3; test_rate_limiter já era). Regressão: roda 5x em sequência (antes falhava na 2ª). Regra: teste que persiste estado em disco do projeto DEVE usar tmp_path — nunca o default de produção (registro de IP real podia bloquear login local no DB compartilhado).
### 115. dependency-audit lock-validation — `*.lock binary` + pip-compile sem --upgrade reusa pins (2026-08-28)
Sintoma: job lock-validation falhava com "Binary files a/requirements.lock and b/requirements.lock differ". Causa dupla: (1) .gitattributes marca *.lock como binary → diff legível impossível; (2) pip-compile SEM --upgrade REUSA os pins do arquivo de saída existente → o job valida CONSISTÊNCIA, nunca atualidade — locks stale vs PyPI não disparam erro, só a divergência header/grupos; (3) pip-tools 7.5.3 passou a injetar --no-index no header, divergindo dos locks commitados. Correção: regen COM --upgrade em WSL (pip 26.1.2 + pip-tools 7.5.3, Python 3.14.6 nativo) → 14+ bumps reais (onnxruntime 1.29.0, pydantic 2.13.5, scipy 1.18.1, sentence-transformers 6.0.0, optimum 1.17.1, cryptography 50.0.1, curl-cffi 0.16.2, torch 2.13.0+cpu, numpy 2.5.2, opencv-python 5.0.0.93); requirements.lock = copy do test. Validado: 1561 unit local + lock-validation verde no PR #76. Latente: optimum 1.17.1 + transformers 5.15.0 quebram o import de optimum.onnxruntime (transformers 5 removeu is_tf_available) — semantic_matcher já cai p/ fallback PyTorch (try/except); export_onnx.py só roda quando optimum suportar transformers 5.

### 116. teste_full_manual — golden em unit fazia rede real + hardcodes drift (2026-08-28)
Sintoma: run 33211265485 falhou em unit (httpx.ConnectError Name or service not known em test_reject_false_positives_golden_300) e em e2e-full (D5 ingredients count == 23 got 27; KeyError "brand" not in index na página Insights). Causa: (1) o golden T1.4 rodava em tests/unit mas chamava get_active_ingredients() real → rede em teste unit viola a regra #3 (Supabase real DEVE usar @pytest.mark.integration) e vira flaky de DNS; (2) D5 hardcodava 23 quando o canônico já é 27 (config/ingredients.yaml); (3) insights.py usava df[[...brand...]] sem garantir a coluna quando nenhum preço carrega brand → KeyError. Correção: golden movido para tests/integration/test_recover_review_queue_golden.py (marcado integration, roda contra DB real em ~1.5s, sem rede na suíte unit); D5 passou a derivar do YAML (len(...)) em vez de hardcode; insights ganhou guard 'if brand not in df.columns: df["brand"] = "Desconhecido"' nos dois pontos. Achado extra (pendência): 21/27 ingredientes têm DB != YAML — DB com MAIS aliases/search_terms que o YAML (refinamentos históricos não sincronizados de volta) — reconciliação fica como follow-up; runtime usa DB (get_active_ingredients) e o golden valida exatamente esse caminho. Teste de regressão: integration golden + suíte unit/dashboard intactos.

### 117. integration paralelas no mesmo Supabase + timeout 15min no full-manual (2026-08-28)
Sintoma: run 33214349310 (CI master do merge) falhou integration com "Price was not created after approve" (test_approve_with_uuid) às 22:00:10Z, enquanto o Teste_Full_Manual 33214410476 rodava integration no MESMO Supabase (pytest iniciado 21:57:40Z) até ser cancelado às 22:10:46Z (timeout-minutes: 15 estourado). Causa: (1) duas suítes tests/integration/ concorrentes no mesmo banco real — os cleanups de test_approve_with_uuid de uma run apagam rows de prices que a outra acabou de upsertar (ON CONFLICT ingredient+store) entre o approve e o assert; (2) full-manual tinha integration timeout 15 (vs 30 no ci.yml) — a suíte cresceu e estourou o limite. Correção: concurrency group "integration-db" (cancel-in-progress: false) no job integration do ci.yml E do teste_full_manual (serializa as suítes no DB real); timeout do full-manual subiu para 30 + passo check_time_budget (paridade com ci.yml); test_approve_with_uuid passou a pré-limpar TODOS os prices da store da fixture (qualquer source) antes de inserir. Teste: integration verdes em sequência (nunca em paralelo no mesmo DB).

### 118. ingredientes YAML vs DB — DB é runtime truth; YAML acumula artifacts de teste de approve (2026-08-28)
Sintoma/causa: 21/27 ingredientes com DB != YAML (DB tem mais aliases/search_terms — refinamentos históricos não sincronizados de volta). `sync_ingredient_fields.py --dry-run` apontou "artifacts de teste nos aliases: 1 [Fermento Químico em Pó] 66 -> 6 aliases" — 60 aliases iniciando "Test Approve UUID <hex> 200g"/"Test Approve Name 500g"/"Test Approve Fuzzy 1kg"/"Duplicate Price*" acumulados das suítes de approve (add_alias_to_ingredient). Runtime usa DB via get_active_ingredients(), então o YAML estava stale/ruído. Union 2-vias (mesclar YAML+DB) quebra o golden (8 divergências, ex.: "Açúcar Azul Arcolor 500g" → "Açúcar Mascavo" 63.5) — direção errada. Correção (RPR): regenerado config/ingredients.yaml a partir do DB ativo (DB→YAML: sobrescrever aliases/brands/search_terms/exclude_terms por ingrediente, preservando ordem do YAML) filtrando prefixos de artifact; `sync_ingredient_fields.py --execute` limpou DB Fermento 66→6 (via RPC 443); --dry-run posterior: 0 defasados + 0 artifacts (idempotente); golden integration (real DB) permaneceu 300/300. Teste de regressão: tests/integration/test_recover_review_queue_golden.py (300/300) + sync --dry-run limpo. Regra: DB é a fonte de verdade do runtime; reconciliação canônica é DB→YAML (NUNCA union 2-vias); após refinar ingredientes no DB, rodar sync --execute para limpar artifacts de teste de approve — não deixar o YAML/stale com aliases de teste.

### 119. OCR de PDF-imagem quebrava com img.tobytes() — cannot identify image file (2026-08-29)
Sintoma: Store Recovery Test #13 (run 33231625239, master 00:31 BRT, 12m1) falhou em `test-store (Atacadão)`: "Page N error: cannot identify image file <_io.BytesIO...>" em todas as páginas, "OCR also returned empty", timeout 300s, 0 produtos (exit 1). Spani passou porque o flyer dela é text-dense (pdfplumber extrai texto); Atacadão é image-based → depende do OCR. Causa raiz: `scrapers/ocr.py::ocr_pdf` e `scrapers/extractor.py::_ocr_pdf` usavam `preprocess_image(img.tobytes())` — `img.tobytes()` do pdf2image/PIL devolve o pixel buffer CRU (sem header de formato) e `Image.open(BytesIO(...))` levanta `cannot identify image file` → OCR nunca funcionava para qualquer PDF-imagem (não era um problema só do Atacadão). `parsers/document_parser.py` já fazia certo (`img.save(buf, "PNG")` antes do preprocess). Correção (RPR): serializar a página como PNG (`buf=io.BytesIO(); img.save(buf, "PNG")`) antes do preprocess nos dois pontos; testes de regressão `tests/unit/test_ocr.py` (2 testes: `ocr_pdf` e `_ocr_pdf` com páginas PIL, patcheando convert_from_bytes + image_to_string). Validado: 1562 unit+schema local, ruff, mypy . (WSL, 78 arquivos). Regra: pdf2image devolve objetos PIL — para serializar use `img.save(obj, "PNG")`, NUNCA `img.tobytes()` (buffer cru sem header não é identificável por `Image.open`).

### 120. Store Recovery Test — tesseract-ocr-por ausente no runner quebrava OCR pt-BR (2026-08-30)
Sintoma: Store Recovery Test #14 (run 33286523269) falhou DE NOVO em `test-store (Atacadão)` mesmo com o fix de OCR #80 (img.tobytes→PNG). O OCR agora rodava, mas todas as páginas davam "Error opening data file /usr/share/tesseract-ocr/5/tessdata/por.traineddata ... Failed loading language 'por' Tesseract couldn't load any languages" → OCR vazio → timeout 300s → 0 produtos. Spani passou de novo (flyer text-dense, sem OCR). Causa (paridade #10): os demais workflows instalam `tesseract-ocr tesseract-ocr-por poppler-utils` (ci.yml:471, scrape-reusable.yml:171, teste_full_manual.yml:190), mas `test_store_recovery.yml:102` usava `tesseract-ocr poppler-utils` SEM `tesseract-ocr-por`. Não há checker automático de pacotes de sistema nos workflows — o gap só aparece em runtime quando uma loja precisa de OCR pt-BR. Correção (RPR simplificado — mudança só de workflow): adicionar `tesseract-ocr-por` ao apt-get. Regra: qualquer alteração em workflow com tesseract/poppler deve conferir os 4 spots (ci.yml, scrape-reusable.yml, teste_full_manual.yml, test_store_recovery.yml) e manter `tesseract-ocr-por`; sem validador automático, revisar manualmente.

### 121. _spawn_isolated — deadlock q.put()/join() com thumbnail grande matava flyer scapers (Atacadão) (2026-08-30)
Sintoma: Store Recovery Test #14 (33286523269) e #15 (33287719788) falharam em `test-store (Atacadão)` mesmo após fix OCR (#80) e tesseract-por (#81): OCR agora extrai 3055 chars e `TIER 1 SUMMARY: matched=20`, mas o processo ficava ~221s SILENCIOSO e o `_spawn_isolated` estourava `TIMEOUT after 300s` → 0 produtos → exit 1. Causa raiz: coletor roda scraper em subprocesso (`_mp_worker`) que devolve via `mp.Queue` o resultado + `_thumbnail` (PNG da 1ª página a 150dpi, >2MB). `_spawn_isolated` ficava em `p.join(timeout)` SEM drenar a queue → filho bloqueava em `q.put()` (pipe cheio) para sempre → join estoura → pai mata o filho e devolve [] — "0 raw products". Provado localmente (script: child põe 2MB, is_alive=True após 3s sem leitor). Spani passava porque cache hit (PDF igual) → run() retorna [] sem thumbnail → payload pequeno. Correção (RPR): `_spawn_isolated` agora drena a queue numa thread daemon (`reader`) desde o start — filho nunca bloqueia; testes `test_collector_isolation.py::test_isolated_big_thumbnail_does_not_deadlock_queue` (thumb 2MB, assert elapsed<10 e len==2M) + 1563 unit+schema + ruff + mypy WSL. Regra: NUNCA `mp.Queue.put()` de payload grande (imagens/thumbnails) sem leitor concorrente no pai — deadlock clássico join×put; drenar sempre.

### 122. #82 foi incompleto — dreno de queue one-shot (15s) re-deadlockava flyer scapers lentos (2026-08-30)
Sintoma: SRT #16 (33289042271) AINDA falhou em Atacadão após o merge do #82 (dreno de queue): mesmo padrão — OCR OK (3055 chars, matched=20 aos ~46s) e depois 254s silenciosos + `TIMEOUT after 300s` → 0 produtos. Causa raiz: o dreno do #82 era um `mp_q.get(timeout=15)` ÚNICO; a thread morria aos 15s com `result` vazio, e o filho só fazia `q.put()` aos ~46s (download+OCR) → sem leitor, `put` bloqueava para sempre → deadlock idêntico ao #121. O teste do #82 (thumb 2MB COM put imediato) não pegava: passava dentro dos 15s. Correção (RPR 2ª iteração): `_drain` agora LOOPS enquanto `p.is_alive()` (`mp_q.get(timeout=1)`, `continue` em Empty; + leitura final curta após o filho sair) — leitor vive a vida inteira do filho. Novos testes `_SlowBigThumbnailScraper` (sleep 25s + thumb 2MB): `test_isolated_delayed_big_thumbnail_does_not_deadlock_queue` (assert 20<elapsed<45) — falharia no one-shot de 15s. Validado: 1564 unit+schema, ruff, mypy WSL. Regra: dreno de mp.Queue em thread precisa durar o ciclo de vida do produtor (loop até is_alive), não um único get com timeout curto; e o teste de regressão DEVE atrasar o put além do timeout do dreno — put imediato não cobre o bug.
