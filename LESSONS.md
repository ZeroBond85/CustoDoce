...
## Security Audit 2026-08 — Lições RPR

### 80. `exec_sql_query` RPC requer `.execute()`
- **Data + commit**: 2026-08-09 (security/audit-2026-08)
- **Sintoma**: `db_security_lint.py --quick` reportava "Todas as verificações passaram" mas havia falhas reais (RLS faltando, SECURITY DEFINER, bucket público)
- **Causa raiz**: `scripts/db_security_lint.py:26` — `client.rpc("exec_sql_query", {"sql": sql})` retorna `SyncRPCFilterRequestBuilder`, não o resultado. Sem `.execute()`, `res.data` não existe e o helper `rpc()` retornava `[]` vazio → checks passavam falso
- **Correção**: `scripts/db_security_lint.py:26` — adicionado `.execute()`: `res = client.rpc("exec_sql_query", {"sql": sql}).execute()`
- **Teste de regressão**: `scripts/db_security_lint.py --quick` agora detecta corretamente falhas

### 81. Secrets em `stores.yaml` sincronizam para DB `config` jsonb
- **Data + commit**: 2026-08-09 (security/audit-2026-08)
- **Sintoma**: `vip_login_key` hardcoded em `config/stores.yaml` (2 lojas, mesmo key) vazava para `stores.config` jsonb via `sync_all_store_fields.py`
- **Causa raiz**: `scripts/sync_all_store_fields.py:72-74` — chaves YAML não em `DB_COLUMNS` vão para `config` jsonb sem filtro de segredo
- **Correção**: 
  1. Removido valor hardcoded de `stores.yaml` (linhas 72, 367)
  2. Scraper lê `os.environ.get("VIP_LOGIN_KEY")` com fallback para config (compatibilidade)
  3. CI workflows passam `VIP_LOGIN_KEY` secret
  3. **Rotação obrigatória** do key no VipCommerce (credencial exposta no histórico git)
- **Teste de regressão**: `tests/unit/test_vipcommerce_api_scraper.py::test_login_key_from_env_has_priority` + `test_login_key_falls_back_to_config_without_env`

### 82. SSRF defense-in-depth via per-redirect re-validation
- **Data + commit**: 2026-08-09 (security/audit-2026-08)
- **Sintoma**: Scrapers usavam `httpx.Client` direto — vulnerável a redirect-based SSRF (CVE-2026-35459)
- **Causa raiz**: `scrapers/base_web_scraper.py:195` criava `httpx.Client` sem validação de redirects
- **Correção**: `services/url_guard.py::make_safe_client()` injeta event hook `response` que re-valida cada hop de redirect contra allowlist + IP público. `BaseWebScraper` e `ExtraFlyerScraper` migrados para `make_safe_client()`
- **Teste de regressão**: `tests/unit/test_services/test_url_guard.py::TestRedirectRevalidation::test_event_hook_wraps_existing_and_blocks_redirect`

### 83. Rate limit + response size limits na base class
- **Data + commit**: 2026-08-09 (security/audit-2026-08)
- **Sintoma**: Scrapers sem throttle consistente nem limite de tamanho de resposta — risco de DoS / memory exhaustion
- **Causa raiz**: `BaseWebScraper` não tinha `_throttle()` automático em `fetch_search`/`fetch_json` nem validação de `Content-Length`
- **Correção**: 
  - `BaseWebScraper`: `DEFAULT_MAX_RESPONSE_SIZE = 10MB`, `_check_response_size()` valida header `Content-Length`; `_throttle()` chamado antes de `fetch_search`/`fetch_json`
  - `ExtraFlyerScraper`: `_throttle()` com `rate_limit` do store config (default 1 req/s)
- **Teste de regressão**: `tests/unit/test_services/test_scraper_base.py::TestBaseWebScraper::test_throttle_positive`

### 84. Hardcoded `verify_ssl: false` em config versionado
- **Data + commit**: 2026-08-09 (security/audit-2026-08)
- **Sintoma**: `config/stores.yaml:146` tinha `verify_ssl: false` para Roldão — desabilita validação TLS
- **Causa raiz**: Config versionada em vez de tratamento no código do scraper
- **Correção**: Removido de `stores.yaml`; `scrapers/roldao_api_scraper.py:12` já força `verify_ssl=False` com comentário explicativo
- **Teste de regressão**: Nenhum (mudança de config)
